...
async function putCats(req, res, next) {
  try {
    let id = req.params.id;
    let catFromReq = req.body;
    let options = {
      new: true,
      overwrite: true
    };

    // findByIdAndUpdate takes in 3 arguements:
    // - 1) id
    // - 2) data object (the new data we want in the database)
    // - 3) options object
    let updatedCat = await Cat.findByIdAndUpdate(id, catFromReq, options);
    // Cat successedfully updated and sent back to front end:
